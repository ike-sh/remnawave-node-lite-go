...
环境变量：
  XRAY_CORE_VERSION   rw-core 版本，默认 v26.7.28（Node 3.4.1 基线）
...
# ASN 前缀数据库（插件 asList 共享列表解析；对齐官方 3.4.1 的 /usr/local/share/asn）。
# Release 归档默认包含官方 ASN index 转换出的紧凑 DB；ASN_DB_URL 可覆盖为自备 .bin。
ASN_DB_URL="${ASN_DB_URL:-}"
ASN_DB_PATH="${ASN_DB_PATH:-/usr/local/share/asn/asn-prefixes.bin}"
install_asn_db() {
  if [ -z "${ASN_DB_URL}" ]; then
    if [ -s "${ASN_DB_PATH}" ]; then
      echo "使用已有 ASN 数据库：${ASN_DB_PATH}"
    else
      echo "提示：未找到 ASN 数据库（插件 asList 将降级为空）。"
    fi
    return 0
  fi
