...
当前版本：**v0.8.4** | 基于官方 `@remnawave/node` v2.7.0 contract（Panel 上报 `nodeVersion=2.7.0`）
...
```bash
curl -fsSL https://raw.githubusercontent.com/ike-sh/remnawave-node-lite-go/v0.8.4/scripts/install-node.sh | sudo bash
```

**256MB VPS 推荐：**

```bash
curl -fsSL https://raw.githubusercontent.com/ike-sh/remnawave-node-lite-go/v0.8.4/scripts/install-node.sh \
...
```bash
curl -fsSL https://raw.githubusercontent.com/ike-sh/remnawave-node-lite-go/v0.8.4/scripts/upgrade.sh | sudo bash
...
```bash
curl -fsSL https://raw.githubusercontent.com/ike-sh/remnawave-node-lite-go/v0.8.4/scripts/uninstall.sh | sudo bash
