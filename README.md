...
当前版本：**v0.8.23** | 基于官方 `@remnawave/node` v2.7.0 contract（Panel 上报 `nodeVersion=2.7.0`）
...
运行后显示菜单：**安装 / 升级 / 卸载 / 退出**（与 `install-node.sh` 一致）。

```bash
# 低内存 VPS（256MB）
bash /tmp/install-alpine.sh --low-memory

# 非交互
SECRET_KEY='eyJ...' bash /tmp/install-alpine.sh --install --yes --port 2222
```
